...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            #para la redireccionamiento a la pagina ckeck
            ...
            try:
                ...
                parans = dict(x.split('=') for x in query.split('&'))
                if 'next' in parans:
                    nextPage = parans['next']
                    return redirect(nextPage)
            except:
                return redirect('dashboard')

        else:
            messages.error(request, 'correo o contraseña incorrecto ')
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.success(request, 'Fue enviado un correo electronico con las instrucciones para resetear tu contraseña')
            return redirect('login')
        else:
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            messages.error(request, 'La contraseña no son iguales')
            return redirect('resetPassword')
    else:
